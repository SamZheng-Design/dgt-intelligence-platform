...
// 生成 interest_alignment 数据
function generateInterestAlignment(deal) {
  const financialDataStr = deal.financial_data;
  let financialData;
  try {
    financialData = typeof financialDataStr === 'string' ? JSON.parse(financialDataStr) : financialDataStr;
  } catch (e) {
    console.error(`  ❌ 无法解析 ${deal.id} 的 financial_data`);
    return null;
  }
  
  const shareRatio = financialData.revenue_share_ratio || deal.revenue_share_ratio || 0.05;
  const period = financialData.investment_period_months || deal.investment_period_months || 24;
  
  return {
    investor_share: shareRatio,
    operator_share: 1 - shareRatio,
    minimum_guarantee: period >= 24,
    minimum_guarantee_amount: period >= 24 ? Math.round(financialData.investment_return?.annual_share_income / 12 * 0.8) || 10000 : null,
    performance_bonus: period >= 30 ? { threshold: 50, bonus_rate: 0.01 } : null,
    lock_up_period_months: Math.min(period / 2, 18),
    exit_mechanism: period >= 36 ? "提前3个月告知，按剩余月份×月均分成80%补偿" : "到期自动结束"
  };
}

// 生成 guarantee_mechanism 数据
function generateGuaranteeMechanism(deal) {
  const financialDataStr = deal.financial_data;
  let financialData;
  try {
    financialData = typeof financialDataStr === 'string' ? JSON.parse(financialDataStr) : financialDataStr;
  ...
  const investmentAmount = financialData.investment_amount || deal.funding_amount || 50;
  const monthlyRevenue = financialData.revenue_data?.monthly_revenue || 100000;
  const shareRatio = financialData.revenue_share_ratio || deal.revenue_share_ratio || 0.05;
  
  return {
    deposit: Math.round(investmentAmount * 0.1),
