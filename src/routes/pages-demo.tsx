  ...
  <div class="flex items-center justify-between">
    <div class="flex items-center space-x-4">
      ...
      <div id="step-2" class="flex items-center space-x-2 opacity-50">
        ...
        <span class="font-medium text-gray-600">外环外环筛子体系</span>
      </div>
      ...
      <div id="step-3" class="flex items-center space-x-2 opacity-50">
        ...
        <span class="font-medium text-gray-600">中环中环漏斗体系</span>
      </div>
  ...
  <div class="lg:col-span-2 space-y-6">
    ...
    <div id="outer-section" class="bg-white rounded-xl card-shadow p-6">
      ...
      <div class="mb-4 p-3 bg-red-50 rounded-lg text-sm text-red-800">
        ...
        外环外环筛子体系采用<strong>串行执行</strong>，每个智能体必须通过才能进入下一步，任何一个不通过则<strong>一票否决</strong>
      </div>
    ...
    <div id="inner-section" class="bg-white rounded-xl card-shadow p-6 opacity-50">
      <div class="flex items-center justify-between mb-4">
        ...
        <span id="inner-status" class="text-sm text-gray-500">等待外环外环筛子体系完成</span>
      </div>
      
      <!-- 中环中环漏斗体系说明 -->
      <div class="mb-4 p-3 bg-[#D9EDDF] rounded-lg text-sm text-[#49754D]">
        <i class="fas fa-info-circle mr-2"></i>
        中环中环漏斗体系采用<strong>并行执行</strong>，多个智能体同时评估，按权重计算加权评分
      </div>
...
<script>
        ...
        <div class="bg-gray-50 rounded-xl p-4">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div class="bg-white p-3 rounded-lg">
              ...
              <span class="font-medium">\${agent.ring_type === 'outer' ? '外环外环筛子体系' : '中环中环漏斗体系'}</span>
            </div>
            <div class="bg-white p-3 rounded-lg">
    ...
    try {
      ...
      for (const agentId of outerAgents) {
        ...
        if (!pass) {
          ...
          updateStep(2, 'error');
          document.getElementById('overall-status').textContent = '外环外环筛子体系未通过';
          showToast('外环外环筛子体系未通过', 'error');
  ...
  function resetDemoForRerun() {
    ...
    document.getElementById('inner-status').textContent = '等待外环外环筛子体系完成';
