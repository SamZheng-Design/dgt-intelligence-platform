    ...
    <div class="flex items-center justify-between">
      <div class="flex">
        <button id="tab-outer" onclick="switchRingTab('outer')" class="px-6 py-4 font-medium text-primary-600 border-b-2 border-primary-500 bg-primary-50">
          <i class="fas fa-filter mr-2"></i>外环外环筛子体系 <span class="ml-2 bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">一票否决</span>
        </button>
        <button id="tab-inner" onclick="switchRingTab('inner')" class="px-6 py-4 font-medium text-gray-500 hover:text-gray-700 transition">
          <i class="fas fa-funnel-dollar mr-2"></i>中环中环漏斗体系 <span class="ml-2 bg-blue-500 text-white text-xs px-2 py-0.5 rounded-full">加权评分</span>
        </button>
      </div>
    </div>
  </div>
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500">所有中环中环漏斗体系智能体一览，按特性标签分类</p>
        </div>
      </div>
      <div class="flex items-center space-x-2">
  ...
  <div class="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto p-6">
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <select id="new-agent-ring" onchange="updateNewAgentForm()" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-400">
              <option value="outer">外环外环筛子体系（一票否决）</option>
              <option value="inner" selected>中环中环漏斗体系（加权评分）</option>
            </select>
          </div>
          <div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">权重 (%) <span id="new-weight-note" class="text-xs text-gray-400">（中环中环漏斗体系有效）</span></label>
        ...
        <div id="panel-config" class="detail-panel hidden">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">外环外环筛子体系智能体权重为0（一票否决制）</p>
            </div>
...
<script>
  ...
  function openAgentModal(id) {
    ...
    document.getElementById('modal-dimension').textContent = currentAgent.dimension + ' | ' + (currentAgent.ring_type === 'outer' ? '外环外环筛子体系' : '中环中环漏斗体系 · ' + trackLabel);
